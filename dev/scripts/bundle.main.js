	            ...
	            $magnific.on('click', function () {
	                ...
	                $.magnificPopup.open({
	                    items: {
	                        src: imgSrc,
	                        type: 'image',
	                        markup: '<div id="popup-mailing" class="popup__mailing mfp-hide">' + '<div class="popup__close">' + '<span class="icon icon--close"></span>' + '</div>' + '<div class="popup__content">' + '<div class="mfp-img"></div>' + '</div>' + '</div>'
	                    }
	                });
	            });
	        });
	    ...
	    $.fn.tabs = function () {
	        ...
	        $(this).each(function () {
	            ...
	            function tabSwitch(numb) {
	                $tabs.removeClass('_active').eq(numb).addClass('_active');
	
	                $btns.removeClass('_active').eq(numb).addClass('_active');
	            }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(function () {
		var whereMapBuyContain = $('.js-wherebuy-map');
		var map = whereMapBuyContain.find('.js-wherebuy-map__map');
		var mapItem = whereMapBuyContain.find('.js-wherebuy-map__map-item');
		var mapItemGroup = whereMapBuyContain.find('.js-wherebuy-map__map-item-group');
		var modal = $('.js-wherebuy-map__modal');
		var modalContent = modal.find('.js-wherebuy-map__modal-content');
		var modalMarkets = modal.find('.js-wherebuy-map__modal-markets');
		...
		function init(data) {
			...
			mapItem.on('mousemove', function (e) {
				var id = $(this).attr('id');
				var current = data.regions['#' + id];
				var html = '<h3>' + current.title + '</h3>';
	
				modalMarkets.find('img').css({ display: 'none' });
				$.each(current.wherebuylist, function (id) {
					var market = current.wherebuylist[id].slice(1);
